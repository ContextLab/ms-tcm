"""MS-TCM Iteration 2: strict hierarchical recall with separate global +
per-storyline contexts.

Extends Iteration 1 (`_likelihood_core_mstcm.py`) with:
...
- K=1: per-storyline = global; reduces to Iteration 1.
...
class V2CoreHyperparams:
    """Scalar MS-TCM Iteration 2 parameters.

    Inherited from C&Z + Iteration 1:
    ...
    @classmethod
    def from_model_parameters(cls, p) -> "V2CoreHyperparams":
        return cls(
...
def run_encoding_v2(
    ...
    """Run W-step Iteration 2 encoding.
    ...
    NOTE: This function is the largest piece of new math in Iteration 2.
...
def _run_encoding_v2_jax(hp, W, K, cat_indices, xp, dtype, e_start):
...
def compute_list_log_likelihood_v2(
    hp: V2CoreHyperparams,
    W: int,
    K: int,
    cat_indices,
    ...
    """End-to-end per-list LL under Iteration 2 (mixture over α/β routes).
    ...
    (c_item_traj_g, c_item_per_s, c_story_per_s, c_list_g_end,
     m_fc_g, m_cf_g, m_fc_s, m_cf_s, m_sc) = run_encoding_v2(
    ...
    if _is_jax(xp):
        ...
        raise NotImplementedError(
            "JAX path for Iteration 2 LL pending; numpy path is correct."
        )
...
def _ll_route_alpha_numpy(
    ...
    for i in range(R):
        ...
        if is_repeat:
            continue  # treat repeats as noise (per Iteration 1 semantics)
...
def compute_list_log_likelihood_v2_numpy(
    params,
    cat_indices,
    recall_sps,
    recall_mask,
    ...
    """Numpy entry point for Iteration 2 list LL."""
    import numpy as np

    hp = V2CoreHyperparams.from_model_parameters(params)
    ll = compute_list_log_likelihood_v2(
        hp, W=W, K=K,
...
def simulate_recalls_v2(
    ...
    *,
    ...
    """Sample a recall sequence from MS-TCM Iteration 2.
