"""Equation-derived oracle tests for MS-TCM Iteration 2
(`_likelihood_core_mstcm_v2`).

Mirrors the test pattern of Iteration 1 but with the new strict-hierarchical
retrieval mechanism + separate global / per-storyline contexts.

Tests:

1. **Reduction-to-Iteration-1**: with K=1 and tau=0, MS-TCM v2 should
   produce the same log-likelihood as the Iteration 1 model
...
3. **Storyline-routes increase pFR at storyline starts**: a sanity
   simulation test (mirrors Iteration 1's analogous test).
"""
...
from ms_tcm._likelihood_core_mstcm_v2 import (
    compute_list_log_likelihood_v2_numpy,
    simulate_recalls_v2,
)
...
# -----------------------------------------------------------------------
# Naive oracle: line-by-line implementation of Iteration 2 equations.
# -----------------------------------------------------------------------
...
def _oracle_v2_encode(p, W, K, cat_indices):
    """Naive numpy reference for Iteration 2 encoding."""
    d = W + 1
...
def _oracle_v2_ll(p, W, K, cat_indices, recall_sps, recall_mask):
    """Naive numpy oracle for Iteration 2 mixture LL."""
    d = W + 1
    ...
    enc = _oracle_v2_encode(p, W, K, cat_indices)
    (c_item_traj_g, c_item_per_s, c_story_per_s, c_list_g,
...
def test_v2_matches_oracle(seed, K, tau, lam):
    """Iteration 2 core LL matches naive oracle to 1e-10."""
    ...
    ll_oracle = _oracle_v2_ll(p, W, K, cat_indices, recall_sps, recall_mask)
    ll_core = compute_list_log_likelihood_v2_numpy(
...
def test_v2_storyline_routes_put_mass_at_storyline_starts():
