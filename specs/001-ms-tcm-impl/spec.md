...
- Q: Should the §4.4 numerical anchor (0.866 grouped / 0.806 bridge) still be tested? → A: Yes, but as a unit test of the model equations (hand-derived, no dataset required), not as a system-level integration test over real data.
...
A memory researcher provides MS-TCM parameters and a dataset in the canonical format; the simulator steps through encoding, scores every recall transition, and emits per-trial and per-condition outputs. For hand-derived inputs the simulator reproduces the §4.4 analytical anchor (composite similarity 0.866 grouped vs 0.806 bridge at β_G = β_S = 0.5, w_G = 0.2, w_S = 0.8, m = 3) to four decimals.
...
**Independent Test**: a unit test runs the composite-similarity computation with hand-constructed unit-norm inputs and the anchor parameters; asserts 0.866 and 0.806 to four decimals. An integration test runs the simulator on a minimal in-memory dataset representing one early and one late list and asserts the expected qualitative ordering.

**Acceptance Scenarios**:

1. **Given** β_G = β_S = 0.5, w_G = 0.2, w_S = 0.8, m = 3, and hand-constructed unit-norm input vectors, **When** the composite-similarity computation is called, **Then** the result equals 0.866 (grouped) and 0.806 (bridge) to four decimals.
...
- **SC-002**: The unit test for the §4.4 numerical anchor asserts composite similarity equals 0.866 (grouped) and 0.806 (bridge) to four decimals at β_G = β_S = 0.5, w_G = 0.2, w_S = 0.8, m = 3 — matching `notes/ms-tcm.pdf` §4.4 exactly.
